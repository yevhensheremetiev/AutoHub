...
export function ServiceDashboardLayout() {
  ...
  useEffect(() => {
    if (!me || me.accountType === 'SERVICE') return;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-50">
      ...
      <div className="container flex flex-1 flex-col pb-24 md:flex-row md:pb-8 md:pt-6">
        ...
        >
          ...
          >
            ...
            {t('service.navBookings')}
          </NavLink>
          <NavLink
            to="/service-dashboard/catalog"
            className={({ isActive }) =>
              cn(navClass, isActive ? navActive : navInactive)
            }
          >
            <ClipboardList className="h-4 w-4 shrink-0" aria-hidden />
            {t('service.navCatalog')}
          </NavLink>
          <NavLink
          ...
          >
            ...
            {t('service.navProfile')}
          </NavLink>
          <NavLink
            to="/service-dashboard/account"
            className={({ isActive }) =>
              cn(navClass, isActive ? navActive : navInactive)
            }
          >
            <User className="h-4 w-4 shrink-0" aria-hidden />
            {t('service.navAccount')}
          </NavLink>
        </nav>
      ...
      <nav
        className="fixed bottom-0 left-0 right-0 z-20 flex border-t border-slate-800/80 bg-slate-950/95 px-1 py-2 backdrop-blur-md md:hidden"
        aria-label={t('service.navAria')}
      >
        <NavLink
          to="/service-dashboard"
